...
function Home() {
  ...
  const [contactPhone, setContactPhone] = useState(() => window.localStorage.getItem('asanib-contact-phone') || '')
  const [shareContactConsent, setShareContactConsent] = useState(false)
  ...
  const canSubmit = useMemo(() => firebaseConfigured && queryText.trim().length >= 8 && location.trim().length >= 2 && phoneDigits.length >= 8 && phoneDigits.length <= 15 && shareContactConsent && (urgency !== 'scheduled' || Boolean(scheduledFor)), [queryText, location, phoneDigits, shareContactConsent, urgency, scheduledFor])
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      window.localStorage.setItem('asanib-contact-phone', contactPhone.trim())
      const parsed = parseDraft({ query: queryText.trim(), location: location.trim(), budget: budget ? Number(budget) : undefined, urgency, scheduledFor: urgency === 'scheduled' ? scheduledFor : undefined, contactPhone: contactPhone.trim(), shareContactConsent })
...
function RequestPage({ requestId }: { requestId: string }) {
  ...
  return <main className="customer-v2-shell"><header className="customer-v2-header"><a className="back-link" href="/">← Home</a><a className="brand" href="/">asanib<span>.</span></a><span /></header><div className="customer-v2-content tab-page">{!request ? <div className="provider-loading">Loading request…</div> : <><div className="request-detail-v2"><div className="request-title-row"><div><span className="eyebrow">{request.urgency === 'now' ? 'NEED IT NOW' : 'SERVICE REQUEST'}</span><h1>{request.query}</h1></div><span className={`status large ${request.status}`}>{request.status}</span></div><div className="request-facts"><div><span>Where</span><strong>{request.location}</strong></div><div><span>Budget</span><strong>{request.budget ? `Up to AED ${request.budget}` : 'Open'}</strong></div><div><span>Service</span><strong>{request.category}</strong></div></div></div>{request.externalMatchStatus === 'provider_interested' && <div className="success-panel"><strong>A business is interested.</strong><p>Asanib has shared your contact details with the matched business. They should contact you directly.</p></div>}{request.externalMatchStatus === 'provider_contacted' && <div className="notice"><strong>Asanib is contacting a local business.</strong> We will keep this request updated as soon as someone responds.</div>}{request.externalMatchStatus === 'needs_match' && <div className="notice">Asanib is looking for someone who can help.</div>}{error && <div className="notice error">{error}</div>}
