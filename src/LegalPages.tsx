...
const sections: Record<LegalKind, { title: string; intro: string; blocks: { heading: string; body: string[] }[] }> = {
  ...
  terms: {
    ...
    blocks: [
      ...
      { heading: '5. Quotes, bookings and payment', body: ['A quote is an offer from a provider. A booking is formed when the customer accepts a quote through Asanib.', 'Where Asanib Checkout is available, Ziina provides the embedded payment interface and JS Ventures LLC receives the customer payment into its Ziina Business account for the Asanib booking. Asanib records the payment status and reference against the booking. Any settlement or amount owed by JS Ventures LLC to the service provider is handled separately from the customer checkout flow.', 'A provider may also offer an approved external checkout destination. Customers choosing an external checkout leave Asanib and pay through the provider or its selected payment processor.'] },
