...
export default function ProviderHubV3() {
  ...
  return <main className="pv3-shell"><aside className="pv3-sidebar"><a className="brand" href="/">asanib<span>.</span></a><div className="pv3-business"><div>{profile?.businessName?.[0]?.toUpperCase() || 'B'}</div><span><strong>{profile?.businessName || 'Your business'}</strong><small>{verificationLabel}</small></span></div><nav>{nav.map(([id, label, icon]) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><i>{icon}</i>{label}{id === 'requests' && matches.length ? <b>{matches.length}</b> : null}</button>)}</nav><div className="pv3-side-bottom"><a href="/">Customer app</a><button onClick={() => void logOut()}>Sign out</button></div></aside>

    <section className="pv3-workspace"><header className="pv3-header"><div><span>ASANIB FOR BUSINESS</span><h1>{tab === 'home' ? `Hi, ${profile?.businessName || 'there'}` : tab === 'requests' ? 'Requests' : tab === 'jobs' ? 'Jobs' : 'Business'}</h1></div><button className={`pv3-availability ${profile?.availableNow ? 'online' : ''}`} disabled={!profile?.approved} onClick={() => void availability()}><i />{profile?.availableNow ? 'Available now' : 'Go available'}</button></header>
